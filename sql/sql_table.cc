...
uint tablename_to_filename(const char *from, char *to, size_t to_length)
{
  ...
  if ((length= check_n_cut_mysql50_prefix(from, to, to_length)))
  {
    ...
    */  
    if (Lex_ident_fs(to, length).check_table_name(true))
    {
      to[0]= 0;
      length= 0;
    }
        ...
        field_visibility_t invisible, Item* default_value)
{
  ...
  {
    fld->field_name= Lex_ident_column(thd->strmake_lex_cstring(field_name));
  }
  ...
  for (uint i=2 ; i< 100; i++)
  {
    *buff_end= '_';
    ...
    if (!check_if_keyname_exists(ident, start, end))
      return Lex_ident_column(thd->strmake_lex_cstring(ident));
  }
